...
/*   Updated: 2023/05/18 21:28:09 by oel-houm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    piping(char **cmd, int infile, int outfile)
{
    ...
    if (pid == 0)
    {
        ...
        close(fd[0]);
        close(fd[1]);
        execvp(cmd[0], cmd); // change this to execve()
    }
    dup2(fd[0], infile);
    close(fd[0]);
    close(fd[1]);
    //waitpid(pid, NULL, 0); // &status of pid here ...
           ...
           // while(s && s[i])
            //{
               ...
               //printf("%d", num_of_cmds);
                while (l < num_of_cmds - 1)
                {
                    out_fd = STDOUT;
                    piping(cmd[l], STDIN, out_fd);
                    l++;
                }
